...
namespace Microsoft.Diagnostics.Tools.Trace
{
    ...
    {
        delegate Task<int> ReportDelegate(CancellationToken ct, IConsole console, int processId, string exec, string providers, string profile, TraceFileFormat format, TimeSpan duration, string clrevents, string clreventlevel, string name);
        ...
        /// <returns></returns>
        private static async Task<int> Report(CancellationToken ct, IConsole console, int processId, string exec, string providers, string profile, TraceFileFormat format, TimeSpan duration, string clrevents, string clreventlevel, string name)
        {
            try
            {
                /*
                ...
                */
                ...
                string diagnosticTransportName = "DOTNET_TOOL_PATH" + Path.GetRandomFileName();
                ...
                DiagnosticsClient diagnosticsClient = new DiagnosticsClient(endpointInfo.Endpoint);
                ...
                using (VirtualTerminalMode vTermMode = VirtualTerminalMode.TryEnable())
                {
                    ...
                    try
                    {
                        session = diagnosticsClient.StartEventPipeSession(providerCollection, true, (int)256);
                    }
                    ...
                    Task monitorTask = Task.Run(() => {
                        try
                        {
                            ...
                            source.Clr.AssemblyLoaderAppDomainAssemblyResolveHandlerInvoked += (AppDomainAssemblyResolveHandlerInvokedTraceData data) => {
                                Console.WriteLine(data.ToString());
                            };
                            source.Clr.AssemblyLoaderAssemblyLoadContextResolvingHandlerInvoked += (AssemblyLoadContextResolvingHandlerInvokedTraceData data) => {
                                Console.WriteLine(data.ToString());
                            };
                            source.Clr.AssemblyLoaderAssemblyLoadFromResolveHandlerInvoked += (AssemblyLoadFromResolveHandlerInvokedTraceData data) => {
                                Console.WriteLine(data.ToString());
                            };
                            source.Clr.AssemblyLoaderKnownPathProbed += (KnownPathProbedTraceData data) => {
                                Console.WriteLine(data.ToString());
                            };
                            source.Clr.AssemblyLoaderResolutionAttempted += (ResolutionAttemptedTraceData data) => {
                                Console.WriteLine(data.ToString());
                            };
                            source.Process();
                        }
        ...
        private static Option FileToExecuteOption() =>
            new Option(
                alias: "--exec",
                description: "The thing to execute")
            {
                Argument = new Argument<string>(name: "exec", defaultValue: "")
            };
